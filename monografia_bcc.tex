...
%FIXED-T: PORTANTO APOS REFEIÇÕES AO PUBLICO DO CAMPUS...
      ...
      \section{Metodologia e Experimentos}
        \subsection{Análises dos dados}
          ...
          Após a estruturação dos dados, será realizada uma análise exploratória fundamentada por \cite{Junior2007}, para previsões de demanda em geral.
          O modelo de dados de venda estruturados receberá o acréscimo de dados exógenos de variáveis climáticas, como possíveis fatores de influencia no consumo, conforme ocorre no trabalho de previsão de demanda de energia elétrica fundamentado por \cite{Almeida2013,RUAS2012,Silva2010}.
          ...
          Assim, a primeira investigação usará a análises estatísticas de regressão linear múltipla fundamentada por \cite{Clarice2011} por meio de métricas aplicadas pelo framework scipy e plot de gráficos scatter por meio da biblioteca seaborn, que além de realizar nos uma construção dos gráficos relacionando os dados exógenos considerados e o comportamento de consumo, obtendo uma regressão linear simples entre estes 2 valores supervisionados, também são retornadas outras métricas como medidas de correlação, p-value, $R^2$ , e outras méticas que serão documentadas para trabalhos futuros.
        ...
        \subsection{Aprendizado de máquina}
          ...
          E também serão considerados modelos modernos, como as redes recorrentes GRU (Gated Recurrent Unit), fundamentados por \cite{DLB}.

          Após a modificação da topologia, os modelos serão duplicados e executados em 2 fases experimentas, sendo a primeira com os experimentos focados para validação e teste no primeiro semestre de 2019, e na segunda fase os experimentos contemplarão o ano de 2018 inteiro para validação e o ano de 2019 inteiro para teste.
          
          Portanto na primeira fase experimental, são considerados 2 modelos MLP para a exploração e aprendizado do problema.
          3 modelos que tem parâmetros de entrada endógenos, e 3 modelos mistos que tem parâmetros endógenos e exógenos de entrada.
          
          Na segunda fase experimental, serão alterados os domínios de treino, validação e teste dos modelos da primeira fase e serão coletados novos resultados.
          
          Estes novos resultados serão comparados com a primeira fase experimental para a tomada de decisão se devemos ter um modelo específico para cada período sazonal do ano (semestres) ou se devemos ter um modelo para todo o período anual.
        
        \section{Resultados} % alterar serao pra sao, sumario de resultados...  %juntar 1.6 com 1.7
        % quais foram os experimentos obtidos, e quais foram os resultados.
            %JUNTAR
            O capítulo de resultados irá demonstrar graficamente as análises de parâmetros de predição com a passagem de alunos no ponto de acesso do restaurante, bem como as predições dos modelos sobre a real passagem.
            Através destas análises gráficas e de valores estatísticos retornados, serão selecionados os melhores modelos para o objetivo final do trabalho que é determinar um modelo de aprendizado de máquina para realizar predições de consumo.
        
        %TODO : VERIFICAR OS INDICES
        \section{Análise dos resultados}
        %JUNTAR
            De acordo com \cite{Flavia2014}, a comparação e a discussão dos resultados de uma predição com redes neurais artificiais deve ser feita com base em medidas de avaliação de erro absoluto médio (EAM), e raiz do erro quadrado médio (REQM). 
            Além destas análises, para o tema deste trabalho, serão considerados os valores de refeições descartadas e em falta (quando o valor previsto é menor que o consumo real), bem como serão documentados outras métricas obtidas pela plataforma experimental (TensorFlow e bibliotecas seaborn, scipy, scikit-learning) para a realização de análises e trabalhos futuros.
  ...
  Este capítulo apresenta toda a fundamentação teórica necessária para o desenvolvimento desta pesquisa. Especificamente, os seguintes tópicos são abordados. Análise de Dados, Modelos de Previsão, ....
      ...
      Na seção de Análises dos Dados, vemos que para realizarmos os experimentos com as redes neurais artificiais com o objetivo de solucionar o problema de predição de refeições no restaurante universitário do ICT Unifesp, serão necessárias algumas fundamentações teóricas para coletarmos e preparamos os dados de entrada nas redes, realizarmos análises exploratórias nestes dados, estipularmos os métodos de previsão, e considerarmos o tratamento dos dados como uma série temporal.
      
      Em seguida fundamentamos a teoria para a construção das redes neurais artificiais, que neste trabalho serão construídas por meio de 2 modelos, as redes neurais MLP (Multilayer Perceptron) e as redes recorrentes GRU (Gated Recurrent Unit).
      ...
      \section{Análises dos Dados}
        \subsection{Tipos de dados}
            Para este trabalho consideramos 2 tipos de dados: Endógenos e Exógenos.
            \paragraph{Endógenos}
            Dados endógenos são os dados do domínio de predição, neste caso, são os valores de consumo de refeições represantado pela quantificação da passagem dos alunos no ponto de acesso, a catraca, do restaurante universitário do ICT Unifesp.
        ...
        \subsection{Componentes Temporais}
          ...
          O cenário deste estudo analisa a sazonalidade da frequência de alunos do ICT - UNIFESP, inferida pelas grades semestrais de suas disciplinas, que consequentemente infere na sazonalidade de consumo de refeições dentro do restaurante universitário.\newline
      ...
      \section{Inteligencia Artificial}
        ...
        \subsection{O Neurônio Artificial Perceptron}
          \textcolor{red}{Em 1958, Rosemblat e outros \cite{Haykin1994}, propos um algoritmo de aprendizagem para o neurônio MCP. O modelo concebido foi denominado Perceptron.}
  ...
  \chapter{Metodologia}
	...
	\section{Pré-processamento}
        ...
        \subsection{Dados exógenos}
            Os dados exógenos a serem correlacionados com o consumo se dividem em 2 tipos principais, os dados climáticos coletados de estações meteorológicas próximas ao ICT Unifesp, e dados derivados das datas dos registros de consumo.
            
            \paragraph{Dados Climáticos}
            	Também serão analisadas variáveis climáticas como dados exógenos, de forma a analisar a influência de fatores externos como temperatura média ambiente, pressão atmosférica, umidade e velocidade do vento. Tais dados podem ser obtidos de forma gratuita pelo BDMEP - Banco de Dados Meteorológicos para Ensino e Pesquisa, pertencente à instituição pública INMET - Instituto Nacional de Meteorologia, pertencente ao MINISTÉRIO DA AGRICULTURA, PECUÁRIA E ABASTECIMENTO do Governo Brasileiro. 
    ...
    \section{Definição e treino dos modelos}
        \subsection{Topologia}
            \paragraph*{Sobre a necessidade de se implementar modelos mistos}
                ...
                Para a saída final pode ser implementado um comitê de redes neurais endógenas e exógenas, com um perceptron na saída, recebendo os 2 valores dos modelos endógenos e exógenos e regredindo as saídas das 2 redes ao valor de saída que será a predição do consumo.
            ...
            \subsubsection{Modelos Mistos : Endógenos e Exógenos}
                \begin{itemize}
                    ...
                    \item  Concatenar a saída das 2 redes neurais em um percetron criando um comitê de redes neurais para obter a saída final prevista.
                \end{itemize}
...
%TODO-T: FIGURA 25 ESTORANDO MARGEM NO TITULO    	
    \subsection{Otimizador}
        ...
        \end{figure}
        Para este trabalho é escolhido o otimizador ADAM no treino de backpropagation.
  ...
  \chapter{Resultados}
    ...
    O parâmetro ``ENTR\_ALMOCO'' da tabela \ref{table:dataset_final} é o atributo alvo da predição, pois se trata do número de entradas no ponto de acesso do restaurante, portanto este parâmetro é frequentemente utilizado nos gráficos comparativos em formato de linha, sendo geralmente comparado com algum outro parâmetro de entrada nos modelos, denominados de ``Features'' neste trabalho, nomenclatura utilizada por reconhecidas referências na área de Data Science, conforme o artigo de \cite{TWDSFeatures}.
    
    Nesta primeira fase também são desenvolvidos 2 modelos de redes MLP para suporte na análise exploratória do problema da predição do restaurante e é verificado se estes modelos tem capacidade de aprendizado sobre o problema.
    
    Depois de verificada a capacidade de aprendizado do problema são desenvolvidos 3 modelos endógenos, sendo uma rede MLP e duas redes GRU. Os resultados de predição são comparados visando a selecionar o melhor modelo endógeno.
    
    Após a seleção do melhor modelo, são desenvolvidos três modelos mistos. Ao contrario dos modeos anteriores, estes modelos recebem como entrada tanto features endógenas quanto exógenas. Da mesma forma, o melhor modelo com ambas as features também é selecionado.
    ...
    Após selecionado o melhor modelo da segunda fase, são comparados os melhores modelos das duas fase. Para uma comparação justa, visto que o melhor modelo da 1a fase tem domínio de teste restrito ao primeiro semestre de 2019, o modelo obtido na segunda fase é novamente testado apenas no domínio do primeiro semestre obtendo novas métricas de teste.
    ...
    %TODO : CITAR O ARTIGO, NÃO A AUTORA QUE NINGUEM CONHECE...
	...
	\section{Fase Experimental I}
    	    ...
    	    \subsubsection{Análise das variáveis endógenas}
        	   ...
        	   % \newpage
        	    ...
        	    \paragraph{Análise da técnica do restaurante}
        	        ...
        	        É possível notar que o modelo do R.U é feito para tolerar descartes devido à multa contratual para falta de refeições e o que a produção de 30\% de refeições acima do consumo da semana anterior, conforme a figura  \ref{fig:case1_ru_pred} produz um comportamento linear distante do comportamento real de consumo, apesar de seguir as tendencias de quedas e aumento de consumo, na figura \ref{fig:case1_ru_pred_scatter} do gráfico scatter gerada pela biblioteca seaborn também demonstra que a regressão linear (representada pela linha vermelha no gráfico) tem o eixo totalmente descentralizado com a função identidade entre o consumo e a predição ideal (representada pela diagonal formada entre a origem do gráfico e o vértice superior direito), gerando também um erro muito maior do que 30\% no somatório total de refeições descartadas no semestre, devido ao comportamento oscilatório do consumo, conforme a tabela \ref{table:case1_rupred}.
                \paragraph{Consumo atual em relação ao consumo do jantar de 1 dia anterior.}
                
                    Apesar de que os alunos que consomem refeições no almoço geralmente são de período e grade horária diferente dos alunos que consomem o jantar no período da noite, nota-se uma relação evidente entre os 2 consumos, evidenciada pelo comportamento linear na figura \ref{fig:case1_consumo_jantar} com correlação (r) = 0.7655, e pela regressão linear entre esses 2 consumos na figura  \ref{fig:case1_consumo_jantar_scatter}.\newline
    	...
    	\subsection{Definição e treino dos modelos}
    	    Nesta seção os modelos de redes neurais foram desenvolvidos, treinados, avaliados sobre o conjunto de validação para obtermos métricas préliminares, e por fim foram testados.
    	   ...
    	   % \newpage
    	    \subsubsection{Ajuste empírico de topologia com os modelos perceptron}
              ...
              \paragraph{MLP1}
                ...
                \end{figure}
    	        É possível notar a diminuição do RMSE (Raiz do erro quadrático médio) ao aumentar a profundidade da rede perceptron para treino e avaliação sob o conjunto de validação. Validando a hipótese de que as predições do consumo no restaurante, pode ser aprendido pelos modelos mais simples de redes neurais e que é possível avançar nas pesquisas com modelos que tragam melhores resultados para as predições. 
          ...
          \subsubsection{Modelos Endógenos}
            ...
            %%%%%%%%%%%%%%%%% RNN ENDO 2
             \paragraph{RNN\_ENDO\_2}
                Definido segundo modelo GRU, RNN\_ENDO\_2, com o aumento da profundidade de unidades do modelo anterior RNN\_ENDO\_1 e com a inclusão do recurso Dropout para eliminar unidades com pesos negativos no grafo denso da rede (Dropout realiza a poda de unidades não relevantes no grafo das unidades).
                \begin{figure}[H]
                  \center{
    	...
    	\subsection{Teste e métricas}
    	    Nesta subseção todos os modelos são testados no conjunto de teste, 1o semestre de 2019. Em práticas com maior volume de modelos, pode-se selecionar apenas os modelos com os melhores resultados na etapa de avaliação, porém para um número de 6 modelos que exige pouco recurso computacional para teste foi considerado o teste para todos os modelos para fins de documentação dos resultados em proveito à trabalhos futuros. 
    	    Entre todos os 6 modelos, é selecionado o melhor modelo endógeno, o melhor exógeno, e ambos serão selecionados para comparações com a fase 2.
        ...
        {\begin{center} \begin{minipage}[c]{1.0\textwidth}
    	    ...
    	    \subsection{Melhor modelo da 1a fase}
                Entre todos os modelos, o melhor modelo, produzindo o menor RMSE, e com vantagem em todas as outras métricas foi o modelo endógeno, RNN\_ENDO\_2, ressaltando que o gráfico, e métricas de predições deste modelo se encontram na figura \ref{fig:case1_rnn_endo2_test}.
                Na ultima seção deste capítulo serão apresentadas todas as métricas de todos os modelos de todas as fases experimentais.
    ...
    \section{2a Fase Experimental}
        ...
        \end{figure}
	    Nesta segunda fase, todos os modelos da fase anterior são importados, treinados e testados sobre um novo domínio temporal de acordo com a figura \ref{fig:case2_timeline}.
        
        % \newpage
	    \subsection{Pré-Processamento}
	        Nesta subseção para a etapa de pré-processamento são avaliados apenas os resultados obtidos da divisão dos conjuntos e da técnica aplicada do restaurante.
    	   ...
    	   % \newpage
    	    ...
    	    {\begin{center}
                ...
                Para a produção de refeições com margem de 30\% acima do consumo do 5o dia anterior, conforme figura \ref{fig:ru_pred} e \ref{fig:ru_pred_scatter} obtemos as seguintes métricas de acordo com a tabela \ref{table:case2_rupred}. É possível notar que o descarte e anomalias de predição tomam grandes proporções a partir do segundo semestre até o fim do ano.
                \begin{table}[!ht]
                \centering
                \caption{Métricas da tecnica do restaurante para a 2a fase.}
